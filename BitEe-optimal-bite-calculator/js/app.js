...
class App {
  constructor() {
    ...
    this.dom = {
      ...
      // Navigation & Supabase Nav
      mobileMenuBtn: document.getElementById('mobileMenuBtn'),
      mobileMenu: document.getElementById('mobileMenu'),
      navHangarBtn: document.getElementById('navHangarBtn'),
      supabaseConfigBtn: document.getElementById('supabaseConfigBtn'),
      ...
      // Actions
      openSaveCloudBtn: document.getElementById('openSaveCloudBtn'),
      openHangarBtn: document.getElementById('openHangarBtn'),
      ...
      // Supabase Modals Cache
      saveCloudModal: document.getElementById('saveCloudModal'),
      closeSaveCloudModalBtn: document.getElementById('closeSaveCloudModalBtn'),
      cancelSaveCloudBtn: document.getElementById('cancelSaveCloudBtn'),
      saveCloudForm: document.getElementById('saveCloudForm'),
      saveSandwichNameInput: document.getElementById('saveSandwichNameInput'),
      saveCreatorNameInput: document.getElementById('saveCreatorNameInput'),
      saveCloudResultArea: document.getElementById('saveCloudResultArea'),
      shareUrlInput: document.getElementById('shareUrlInput'),
      copyShareUrlBtn: document.getElementById('copyShareUrlBtn'),

      hangarModal: document.getElementById('hangarModal'),
      closeHangarModalBtn: document.getElementById('closeHangarModalBtn'),
      refreshHangarBtn: document.getElementById('refreshHangarBtn'),
      hangarOpenSaveBtn: document.getElementById('hangarOpenSaveBtn'),
      hangarOpenConfigBtn: document.getElementById('hangarOpenConfigBtn'),
      hangarCardsGrid: document.getElementById('hangarCardsGrid'),
      globalTotalCount: document.getElementById('globalTotalCount'),
      globalAvgEfficiency: document.getElementById('globalAvgEfficiency'),
      globalStallsCount: document.getElementById('globalStallsCount'),

      supabaseConfigModal: document.getElementById('supabaseConfigModal'),
      closeSupabaseConfigModalBtn: document.getElementById('closeSupabaseConfigModalBtn'),
      cancelSupabaseConfigBtn: document.getElementById('cancelSupabaseConfigBtn'),
  ...
  async init() {
    ...
    // Check URL parameters for deep-linking (e.g. ?id=uuid or ?preset=breadMonster&aero=1)
    const urlParams = new URLSearchParams(window.location.search);
    const idParam = urlParams.get('id') || urlParams.get('share');
    ...
    if (idParam) {
      try {
        const cloudSandwich = await supabaseDb.getSandwichById(idParam);
        if (cloudSandwich) {
          this.layers = typeof cloudSandwich.layers === 'string' ? JSON.parse(cloudSandwich.layers) : cloudSandwich.layers;
          this.currentSandwichName = cloudSandwich.name || "Cloud Airframe";
          this.showToast(`Loaded "${this.currentSandwichName}" from Supabase!`);
        }
      } catch (e) {
        console.warn("Could not load cloud sandwich:", e);
      }
    } else if (layersParam) {
      try {
        const decodedLayers = JSON.parse(decodeURIComponent(layersParam));
        if (Array.isArray(decodedLayers)) {
          this.layers = decodedLayers;
          this.currentSandwichName = "Shared Airframe";
          this.showToast("Loaded shared custom airframe!");
        }
      } catch (e) {
        console.warn("Could not parse layers param:", e);
      }
    ...
    } else if (modalParam === 'hangar') {
      if (this.dom.openHangarBtn) this.dom.openHangarBtn.click();
    }
  }
  ...
  bindEvents() {
    ...
    // 13. Supabase Cloud Save Modal
    if (this.dom.openSaveCloudBtn) {
      this.dom.openSaveCloudBtn.addEventListener('click', () => {
        sound.click();
        this.dom.saveSandwichNameInput.value = this.currentSandwichName;
        this.dom.saveCloudResultArea.classList.add('hidden');
        this.dom.saveCloudModal.classList.remove('hidden');
        this.dom.saveSandwichNameInput.focus();
      });
    }

    if (this.dom.closeSaveCloudModalBtn) {
      this.dom.closeSaveCloudModalBtn.addEventListener('click', () => {
        this.dom.saveCloudModal.classList.add('hidden');
      });
    }

    if (this.dom.cancelSaveCloudBtn) {
      this.dom.cancelSaveCloudBtn.addEventListener('click', () => {
        this.dom.saveCloudModal.classList.add('hidden');
      });
    }

    if (this.dom.saveCloudForm) {
      this.dom.saveCloudForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        sound.click();
        const submitBtn = document.getElementById('submitSaveCloudBtn');
        if (submitBtn) {
          submitBtn.disabled = true;
          submitBtn.innerHTML = '<span>⏳ Uploading...</span>';
        }

        const name = this.dom.saveSandwichNameInput.value.trim() || this.currentSandwichName;
        const creator = this.dom.saveCreatorNameInput.value.trim() || "Anonymous Sandwich Engineer";

        const saveRes = await supabaseDb.saveSandwich({
          name: name,
          creator: creator,
          efficiency: this.analysis.efficiency.score,
          biteAngle: this.analysis.biteAngle,
          totalThickness: this.analysis.totalThickness,
          flavorLift: this.analysis.flavorLift,
          layers: this.layers
        });

        if (submitBtn) {
          submitBtn.disabled = false;
          submitBtn.innerHTML = '<span>🚀 Publish to Supabase</span>';
        }
        ...
        if (saveRes && saveRes.success) {
          ...
          let shareUrl = "";
          if (saveRes.isCloud) {
            shareUrl = `${origin}?id=${saveRes.id}`;
          } else {
            shareUrl = `${origin}?layers=${saveRes.encodedParam}`;
          }

          this.dom.shareUrlInput.value = shareUrl;
          this.dom.saveCloudResultArea.classList.remove('hidden');
          sound.targetLock();
          this.showToast("Sandwich published! Share link generated.");
        }
      });
    }
    ...
    if (this.dom.copyShareUrlBtn) {
      this.dom.copyShareUrlBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          this.showToast("Could not copy URL.");
        }
      });
    }

    // 14. Community Airframe Hangar Modal
    const openHangar = async () => {
      sound.click();
      this.dom.hangarModal.classList.remove('hidden');
      await this.renderHangar();
    };

    if (this.dom.openHangarBtn) this.dom.openHangarBtn.addEventListener('click', openHangar);
    if (this.dom.navHangarBtn) this.dom.navHangarBtn.addEventListener('click', openHangar);

    if (this.dom.closeHangarModalBtn) {
      this.dom.closeHangarModalBtn.addEventListener('click', () => {
        this.dom.hangarModal.classList.add('hidden');
      });
    }

    if (this.dom.refreshHangarBtn) {
      this.dom.refreshHangarBtn.addEventListener('click', async () => {
        sound.click();
        await this.renderHangar();
        this.showToast("Hangar data refreshed from Supabase");
      });
    }

    if (this.dom.hangarOpenSaveBtn) {
      this.dom.hangarOpenSaveBtn.addEventListener('click', () => {
        this.dom.hangarModal.classList.add('hidden');
        if (this.dom.openSaveCloudBtn) this.dom.openSaveCloudBtn.click();
      });
    }

    if (this.dom.hangarOpenConfigBtn) {
      this.dom.hangarOpenConfigBtn.addEventListener('click', () => {
        this.dom.hangarModal.classList.add('hidden');
        if (this.dom.supabaseConfigBtn) this.dom.supabaseConfigBtn.click();
      });
    }

    // 15. Supabase Connection Settings Modal
    if (this.dom.supabaseConfigBtn) {
      this.dom.supabaseConfigBtn.addEventListener('click', () => {
        sound.click();
        const savedUrl = localStorage.getItem('bitee_supabase_url') || "";
        const savedKey = localStorage.getItem('bitee_supabase_key') || "";
        this.dom.supabaseUrlInput.value = savedUrl;
        this.dom.supabaseKeyInput.value = savedKey;
        this.dom.supabaseConfigModal.classList.remove('hidden');
      });
    }

    if (this.dom.closeSupabaseConfigModalBtn) {
      this.dom.closeSupabaseConfigModalBtn.addEventListener('click', () => {
        this.dom.supabaseConfigModal.classList.add('hidden');
      });
    }

    if (this.dom.cancelSupabaseConfigBtn) {
      this.dom.cancelSupabaseConfigBtn.addEventListener('click', () => {
        this.dom.supabaseConfigModal.classList.add('hidden');
      });
    }

    if (this.dom.clearSupabaseConfigBtn) {
      this.dom.clearSupabaseConfigBtn.addEventListener('click', () => {
        sound.click();
        supabaseDb.clearConfig();
        this.dom.supabaseUrlInput.value = "";
        this.dom.supabaseKeyInput.value = "";
        this.dom.supabaseConfigModal.classList.add('hidden');
        this.showToast("Supabase reset to sandbox mock mode");
      });
    }

    if (this.dom.supabaseConfigForm) {
      this.dom.supabaseConfigForm.addEventListener('submit', (e) => {
        e.preventDefault();
        sound.click();
        const url = this.dom.supabaseUrlInput.value.trim();
        const key = this.dom.supabaseKeyInput.value.trim();
        const configured = supabaseDb.saveConfig(url, key);
        this.dom.supabaseConfigModal.classList.add('hidden');
        if (configured) {
          sound.targetLock();
          this.showToast("⚡ Supabase Cloud Connected Successfully!");
        } else {
          this.showToast("Settings saved. Verifying Supabase connection...");
        }
      });
    }
  }
  ...
  /**
   * Renders the Community Airframe Hangar with Supabase items
   */
  async renderHangar() {
    this.dom.hangarCardsGrid.innerHTML = '<div class="col-span-full text-center py-8 font-mono text-xs text-slate-500">Scanning Supabase cloud hangar...</div>';

    const [sandwiches, telemetry] = await Promise.all([
      supabaseDb.getCommunitySandwiches(15),
      supabaseDb.getGlobalTelemetry()
    ]);

    if (this.dom.globalTotalCount) this.dom.globalTotalCount.textContent = telemetry.totalSandwiches.toLocaleString();
    if (this.dom.globalAvgEfficiency) this.dom.globalAvgEfficiency.textContent = telemetry.avgEfficiency;
    if (this.dom.globalStallsCount) this.dom.globalStallsCount.textContent = telemetry.stallsPrevented.toLocaleString();

    this.dom.hangarCardsGrid.innerHTML = '';

    if (!sandwiches || sandwiches.length === 0) {
      this.dom.hangarCardsGrid.innerHTML = '<div class="col-span-full text-center py-8 text-slate-400 font-mono text-xs">No community airframes found. Be the first to publish one!</div>';
      return;
    }

    sandwiches.forEach(item => {
      const card = document.createElement('div');
      card.className = "p-4 rounded-xl bg-slate-900/90 border border-slate-800 hover:border-cyan-500/50 transition-all flex flex-col justify-between shadow-lg";

      const layers = typeof item.layers === 'string' ? JSON.parse(item.layers) : item.layers;
      const layerIcons = Array.isArray(layers) ? layers.map(l => l.icon || '▫️').slice(0, 6).join(' ') : '🥪';

      const effColor = item.efficiency >= 90 ? "text-emerald-400" : item.efficiency >= 75 ? "text-cyan-400" : item.efficiency >= 50 ? "text-yellow-400" : "text-red-400";

      card.innerHTML = `
        <div>
          <div class="flex items-center justify-between pb-2 border-b border-slate-800">
            <span class="text-xs font-mono text-slate-400 truncate max-w-[140px]">👤 ${item.creator_name || 'Anonymous'}</span>
            <span class="text-xs font-mono font-extrabold ${effColor}">${item.efficiency}% EFF</span>
          </div>

          <h4 class="font-heading font-bold text-white text-sm mt-2 truncate">${item.name}</h4>
          
          <div class="text-base my-2 tracking-wider select-none">${layerIcons}</div>

          <div class="grid grid-cols-2 gap-2 text-[11px] font-mono text-slate-400 my-2 bg-space-950 p-2 rounded-lg border border-slate-800/80">
            <div>Angle: <span class="text-cyan-300 font-bold">${item.bite_angle || 15}°</span></div>
            <div>Thick: <span class="text-amber-300 font-bold">${item.total_thickness || 45}mm</span></div>
          </div>
        </div>

        <div class="pt-2 mt-2 border-t border-slate-800 flex items-center justify-between">
          <button class="upvote-btn text-xs font-mono text-slate-400 hover:text-amber-300 flex items-center space-x-1" data-id="${item.id}">
            <span>🚀</span>
            <span class="upvote-count">${item.upvotes || 0}</span>
          </button>

          <button class="load-hangar-btn px-3 py-1 bg-cyan-600 hover:bg-cyan-500 text-white rounded font-mono font-bold text-xs shadow transition-colors" data-id="${item.id}">
            Load Airframe
          </button>
        </div>
      `;

      // Event bindings inside hangar card
      const loadBtn = card.querySelector('.load-hangar-btn');
      loadBtn.addEventListener('click', () => {
        sound.targetLock();
        this.layers = layers;
        this.currentSandwichName = item.name;
        this.update(true, { forceCelebrate: item.efficiency >= 90 });
        this.dom.hangarModal.classList.add('hidden');
        this.showToast(`Loaded "${item.name}" into builder!`);
        const builderEl = document.getElementById('builderSection');
      ...
      upvoteBtn.addEventListener('click', async () => {
        ...
        const curr = parseInt(countSpan.textContent) || 0;
        countSpan.textContent = curr + 1;
        upvoteBtn.classList.add('text-amber-400');
        await supabaseDb.upvoteSandwich(item.id);
      });

      this.dom.hangarCardsGrid.appendChild(card);
    });
  }
